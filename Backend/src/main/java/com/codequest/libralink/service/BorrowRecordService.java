...
public class BorrowRecordService {
    ...
    public BorrowRecord saveRecord(BorrowRecord rec) {
        ...
        rec.setBook(book);
        rec.setSchoolId(book.getInstitution() != null ? book.getInstitution().getInstitutionId() : null);
    ...
    public BorrowRecord checkInCopy(BookCopy copy) {
        ...
        if (dueDate != null && dueDate.isBefore(LocalDate.now()) && ownerId != null) {
            ...
            upsertOverdueFine(saved.getId(), ownerId, book.getTitle(), daysLate,
                    book.getInstitution() != null ? book.getInstitution().getInstitutionId() : null);
        }

        if (saved.getUser() != null) {
            Notification notification = new Notification();
            notification.setUserId(saved.getUser().getId());
            notification.setSchoolId(book.getInstitution() != null ? book.getInstitution().getInstitutionId() : null);
    ...
    private void upsertOverdueFine(Integer borrowId, Integer userId, String bookTitle, long daysLate,
        ...
        if (existing != null) {
            ...
            // schoolId set explicitly here (rather than left for FineService.createFine to
            // resolve from the request's SchoolContext) because this path also runs from
            // OverdueBookScheduler's background job, which has no authenticated caller.
            Fine fine = new Fine();
    ...
    public boolean assessOverdue(Integer recordId) {
        ...
        if (ownerId != null) {
            upsertOverdueFine(record.getId(), ownerId, title, daysLate, record.getSchoolId());
        }
